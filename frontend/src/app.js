...
export const appOptions = {
  ...
  methods: {
    ...
    navigate(view){this.previousView=['chat','ai-detail','settings','usage'].includes(this.activeView)?this.previousView:this.activeView;this.activeView=view;this.searchQuery='';this.$nextTick(this.scrollChatToBottom);},
    goBackSecondary(){this.activeView=this.previousView||'mine';},
    focusSearch(){this.$nextTick(()=>this.$refs.searchInput?.focus());},
    openThread(type,id){this.activeChatRef={type,id};const item=(type==='group'?this.groups:this.companions).find(x=>x.id===id);if(item)item.unread=false;this.persist();if(window.innerWidth<=760||this.activeView!=='inbox'){this.previousView=this.activeView;this.activeView='chat';}this.$nextTick(this.scrollChatToBottom);},
    ...
    removeCompanion(companion){if(!companion||!confirm(`确定删除 ${companion.name} 吗？相关聊天和记忆也会从本机移除。`))return;this.companions=this.companions.filter(x=>x.id!==companion.id);this.groups.forEach(g=>g.memberIds=(g.memberIds||[]).filter(id=>id!==companion.id));this.activeChatRef=null;this.persist();this.activeView='studio';this.showToast('AI 好友已删除');},
    ...
    removeGroup(group){if(!confirm(`确定删除群聊“${group.name}”吗？`))return;this.groups=this.groups.filter(x=>x.id!==group.id);this.persist();this.showToast('群聊已删除');},
