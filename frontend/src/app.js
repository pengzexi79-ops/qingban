...
const ICON_PATHS = {
  ...
  settings: '<circle cx="12" cy="12" r="2.8"/><path d="M17.2 12h3.4M12 17.2v3.4M6.8 12H3.4M12 6.8V3.4M15.68 15.68l2.4 2.4M8.32 15.68l-2.4 2.4M8.32 8.32 5.92 5.92M15.68 8.32l2.4-2.4"/>',
  moments: '<path d="M14.5 4h-5L7 7H4a2 2 0 0 0-2 2v9a2 2 0 0 0 2 2h16a2 2 0 0 0 2-2V9a2 2 0 0 0-2-2h-3l-2.5-3z"/><circle cx="12" cy="13" r="3"/>',
...
const ChatView = {
  ...
  template: `<div class="chat-view">
    ...
    <div class="chat-memory-hint"><span v-html="icon(thread.type === 'group' ? 'group' : 'brain')"></span><span>{{ thread.type === 'group' ? '多 AI 发言调度为前端模拟，等待后端实现' : '长期记忆会按角色设置注入上下文' }}</span><button @click="$emit('open-settings')">查看配置 <span v-html="icon('arrow-right')"></span></button></div>
     <div class="chat-messages"><div v-if="!messages.length" class="chat-empty"><div class="empty-orb"><span v-html="icon('message')"></span></div><strong>从一句自然的话开始</strong><p>这是可交互前端原型。真实模型、记忆检索和主动消息需要后端接入。</p><button class="secondary-action" @click="$emit('quick','今天过得有点复杂，想和你聊聊。')">发个招呼</button></div><div v-for="(message,index) in messages" :key="message.id" class="message-block" :class="{mine:message.role==='user'}"><div v-if="index===0" class="date-divider">今天</div><div class="message-line"><div v-if="message.role!=='user'" class="message-avatar avatar" :style="avatarStyle(sender(message))"><span>{{ sender(message).initial || sender(message).name.slice(0,1) }}</span><img v-if="sender(message).avatarImage" :src="sender(message).avatarImage" alt="" @error="$event.target.remove()"/></div><div class="message-body"><small v-if="thread.type==='group'&&message.role!=='user'" class="message-sender">{{ sender(message).name }}</small><div class="message-bubble">{{ message.content }}</div><time>{{ formatMessageTime(message.timestamp) }}<span v-if="message.proactive" class="proactive-tag">主动消息</span></time></div></div></div></div>
...
export const appOptions = {
  ...
  data() { return {
    ...
    showGroupDialog: false, showProfileDialog: false, showMemoryDialog: false, showMomentDialog: false, showAbout: false, utilityPanel: '', editingMemory: null, companionForm: {}, groupForm: {}, profileForm: {}, memoryForm: {}, momentForm: {content:'',visibility:'所有好友',imageTone:''}, commentDrafts: {}, toastMessage: '', toastTimer: null, persistTimer: null, confirmDialog: { show: false, title: '', message: '', confirmText: '确定', cancelText: '取消', tone: 'danger', resolve: null },
    providerOptions: [
  ...
  methods: {
    ...
    async removeCompanion(companion){if(!companion)return;if(!await this.askConfirm({title:'删除 AI 好友',message:`确定删除“${companion.name}”吗？相关聊天和记忆也会从本机移除。`,confirmText:'删除好友'}))return;this.companions=this.companions.filter(x=>x.id!==companion.id);this.groups.forEach(g=>g.memberIds=(g.memberIds||[]).filter(id=>id!==companion.id));this.activeChatRef=null;this.selectedCompanionId=this.companions[0]?.id||'';this.persist();this.addMenuOpen=false;this.activeView=this.previousView==='chat'?'inbox':(['inbox','contacts','studio'].includes(this.previousView)?this.previousView:'contacts');this.showToast('AI 好友已删除');},
    async handleImageUpload(event,target){const input=event.target,file=input.files?.[0];input.value='';if(!file)return;const allowed=['image/jpeg','image/png','image/webp','image/gif'];if(!allowed.includes(file.type))return this.showToast('仅支持 JPG、PNG、WebP、GIF 图片');if(file.size>5*1024*1024)return this.showToast('图片不能超过 5 MB');try{const image=await this.compressAvatar(file);if(target==='companion')this.companionForm.avatarImage=image;if(target==='group')this.groupForm.avatarImage=image;if(target==='user')this.profileForm.userAvatar=image;this.showToast('头像已更新，保存后生效');}catch(e){this.showToast('头像处理失败，请更换图片');}},
    async changeUserAvatar(event){const input=event.target,file=input.files?.[0];input.value='';if(!file)return;const allowed=['image/jpeg','image/png','image/webp','image/gif'];if(!allowed.includes(file.type))return this.showToast('仅支持 JPG、PNG、WebP、GIF 图片');if(file.size>5*1024*1024)return this.showToast('图片不能超过 5 MB');try{const image=await this.compressAvatar(file);this.settings.userAvatar=image;this.persist();this.showToast('头像已更换');}catch(e){this.showToast('头像处理失败，请更换图片');}},
    compressAvatar(file){return new Promise((resolve,reject)=>{const r=new FileReader();r.onerror=reject;r.onload=()=>{const image=new Image();image.onerror=reject;image.onload=()=>{const size=320,c=document.createElement('canvas');c.width=size;c.height=size;const x=c.getContext('2d');if(!x)return reject();const scale=Math.max(size/image.width,size/image.height),w=image.width*scale,h=image.height*scale;x.imageSmoothingEnabled=true;x.imageSmoothingQuality='high';if(file.type!=='image/png'){x.fillStyle='#eef0f7';x.fillRect(0,0,size,size);}x.drawImage(image,(size-w)/2,(size-h)/2,w,h);resolve(file.type==='image/png'?c.toDataURL('image/png'):c.toDataURL('image/jpeg',.88));};image.src=r.result;};r.readAsDataURL(file);});},
    ...
    async removeMemory(memory){if(!await this.askConfirm({title:'删除记忆',message:'确定删除这条长期记忆吗？',confirmText:'删除'}))return;const c=this.companions.find(x=>x.id===(memory.companion?.id||memory.companionId));if(!c)return;c.memories=c.memories.filter(x=>x.id!==memory.id);this.persist();this.showToast('记忆已删除');},
    ...
    async removeApiProfile(profile){if(this.apiProfiles.length<=1)return this.showToast('至少保留一套 API 配置');if(!await this.askConfirm({title:'删除 API 配置',message:`删除“${profile.name}”吗？`,confirmText:'删除'}))return;this.apiProfiles=this.apiProfiles.filter(x=>x.id!==profile.id);this.selectedApiId=this.apiProfiles[0].id;this.companions.forEach(x=>{if(x.apiProfileId===profile.id)x.apiProfileId=this.selectedApiId;});this.persist();},
    ...
    async resetDemo(){if(!await this.askConfirm({title:'恢复演示数据',message:'恢复演示数据会覆盖当前浏览器中的亲伴数据，确定继续吗？',confirmText:'恢复'}))return;const s=QinbanStore.reset();this.companions=s.companions;this.groups=s.groups;this.moments=s.moments;this.apiProfiles=s.apiProfiles;this.favorites=s.favorites;this.settings=s.settings;this.selectedApiId=s.apiProfiles[0].id;this.activeChatRef=null;this.showToast('演示数据已恢复');},
    async clearAllData(){if(!await this.askConfirm({title:'清空本地数据',message:'确定清空所有本地演示数据吗？此操作不可恢复。',confirmText:'清空'}))return;this.companions=[];this.groups=[];this.moments=[];this.favorites=[];this.persist();this.activeChatRef=null;this.showToast('本地业务数据已清空');},
    ...
    askConfirm({title='确认操作',message='',confirmText='确定',cancelText='取消',tone='danger'}={}){return new Promise(resolve=>{this.confirmDialog={show:true,title,message,confirmText,cancelText,tone,resolve};});},
    resolveConfirm(result){const d=this.confirmDialog;this.confirmDialog={show:false,title:'',message:'',confirmText:'确定',cancelText:'取消',tone:'danger',resolve:null};if(d&&d.resolve)d.resolve(result);}
  }
};
