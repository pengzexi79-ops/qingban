...
### 正态分布随机主动消息(方案定稿 2026-09-05)

> 目标:主动消息"像真人想起用户"——触发时刻不随机乱撞,而是按**用户本人的聊天习惯**拟合正态分布后再随机;默认配置只在冷启动/样本不足时兜底。

1. **习惯计算(数据 → 参数)**
   - 样本:近 30 天 messages 中 ole=user 的消息,取**用户"开启对话"的时刻**(每个会话内第一条用户消息),不含 AI 消息与主动消息;
   - 拟合:对"开启时刻"的小时分布取峰值时段为 μ,用稳健离散度(MAD 换算)为 σ;默认先不区分工作日/周末(留开关);
   - **回落**:有效样本天数不足 10(或样本过少)→ 用默认配置(时间窗 + 窗中点),审计标注 mode=default,积累足够后再切 mode=learned。
2. **每日计划生成**
   - 条数 N:由 requency 档位给出目标条数,当天 ±1 抖动,且不超 dailyLimit;
   - 采样:以 N(μ,σ) 用 Box–Muller 抽 N 个时刻 → 裁剪进时间窗 start~end、过滤免打扰/忙碌 → 两两间隔不足 minMinutes 的重抽;
   - 计划存储:落轻量表跨重启(推荐默认;若走易失缓存需重启后当天重排)。
3. **触发即时校验(与实时规则一致)**:用户总开关、角色 proactive.enabled、免打扰、进行中对话(2s 心跳/刚发消息/AI 回复中)命中则顺延;每次触发带幂等键并落审计(proactive=true),可查看/关闭/删除。
4. **重算节奏**:每日滚动重算一次(窗口 30 天),习惯变化随记录自动适应。
5. **立即触发接口**:POST /companions/{id}/proactive 走同一套校验,时刻=now(合规时)。
