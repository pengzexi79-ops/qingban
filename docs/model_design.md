...
- **删除** `apiprofile.go`、`session.go`,并在迁移中**删除旧表 `api_profiles`**(不搬运数据):
  `ApiProfile` 已标记废弃,和 `APIConfig` 职责完全重复,服务层引用随提交一并改成 `APIConfig`;
  "会话"统一由 `Conversation` 承担;会话用什么模型配置改由外键表达——
  单聊看角色上的 `api_config_id`,群聊里每个成员自己带 `api_config_id`(谁发言用谁的配置)。
...
> 注:群"回合/轮次"不在图中——回合是**运行期易失态**(见 3.3),由进程内缓存承载,不建表。
...
删除规则(由外键约束落实):删角色 → 级联删其会话 → 级联删消息、点名、附件关系、已读水位、短期记忆;
...
- **回合是什么**:手动触发"一轮"(或点名全员)是一次**运行期调度轨迹**(谁被喂、谁回复、产出哪些消息),不是需要存档的业务数据 → **不建表**,取消原 `rounds/round_speakers` 设计(AutoMigrate 清单同步删除;旧表由迁移脚本 DROP,`groups.last_round_at` 列一并删除)。
- **放在哪**:进程内易失 KV 缓存(`bigcache/v3`,见 `core/cache.go`,`core.Mem`):
  - `round:<conversation_id>` → 运行中回合现场(回合号/目标成员/已产出消息 id),回复完成即删;
  - `flush:<conversation_id>` → 上次投喂开始时刻(群冷却判断);
  - `act:<conversation_id>` → 用户"最近活动/打字心跳"时刻(见下方输入完成判定)。
- **对外表现**:`POST /groups/:id/rounds` 立即返回运行期回合号,过程经 SSE(`round_start → round_message → round_end`)推送;**无历史回放接口**(以后需要"群聊回顾"用消息表现查即可)。重启即失,可接受(回合本无跨重启语义)。

### 3.4 用户输入完成判定(防"逐条秒回")

- 目的:AI 应像真人一样**等对方把话说完再回**,不因每条消息到达都立刻回复。
- 规则(事件驱动,无常驻扫描):
  1. 用户**发出消息后**,进入"判断是否继续说"的窗口;
  2. 期间每来一条新消息、或前端**"正在输入"心跳**(`POST /conversations/:id/typing`,打字中周期上报),窗口**滚动重置**;
  3. 窗口上限 = **`core.IdleWindow`(默认 2 秒,core 包编译期可注入变量,装配/测试可覆盖)**;超时无任何活动 → 判定"说完了" → 才把攒批内容一次性投喂给 AI;
  4. **AI 完成一次回复后,本轮等待/判断即终止**,清空 `act:` 现场,回到"等用户下一条消息"状态;群聊点名(@)可走立即投喂但仍尊重该窗口。
- 缓存位置与持久层分工:`core.Mem`(易失)只放活动/冷却/回合;**必须跨重启的**(消息、会话、角色、API 配置、已读水位、记忆)一律 SQLite;`config_kvs` 是持久加密键值,与 `core.Mem` 易失缓存是两回事。
...
- **模型配置(api_configs)**:自增主键;`唯一名/显示名/描述/版本`;
...
| 位置 | 动作 |
|---|---|
| 服务层 | 废弃实体改名成 `APIConfig`;会话缓存读写改成"会话行 + 整表预载";去掉对旧配置实体的依赖 |
| 智能体层(注释草稿) | 回合/冷却改走 core.Mem(易失缓存);点名/成员语义按新模型对齐 |
